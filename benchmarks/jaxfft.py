...
def run_benchmark(pdims, global_shape, backend, nb_nodes, precision, iterations, trace, contiguous, output_path):
    ...
    array = jax.random.normal(shape=[global_shape[0] // pdims[0], global_shape[1] // pdims[1], global_shape[2]], key=jax.random.PRNGKey(rank))
...
if __name__ == '__main__':
    ...
    parser.add_argument('-g', '--global_shape', type=int, help='Global shape of the array', default=None)
    parser.add_argument(
        '-b', '--backend', type=str, help='Backend to use for transpose comm', choices=['NCCL', 'NCCL_PL', 'MPI_P2P', 'MPI'], default='NCCL'
    )
    ...
    run_benchmark(pdims, global_shape, backend, nb_nodes, args.precision, args.iterations, args.trace, args.contiguous, output_path)
