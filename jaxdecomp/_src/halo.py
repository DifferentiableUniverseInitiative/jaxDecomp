...
def inner_halo_abstract_eval(x, halo_extents, halo_periods, reduce_halo, pdims,
                             global_shape):
...
def inner_halo_lowering(ctx, x, *, halo_extents, halo_periods, reduce_halo,
                        pdims, global_shape):
  (x_aval,) = ctx.avals_in
...
def partition(halo_extents, halo_periods, reduce_halo, mesh, arg_shapes,
  ...
  def lower_fn(operand):
    ...
    if reduce_halo:
      ...
      output = output.at[halo_extents[0]:2 * halo_extents[0]].add(
          output[:halo_extents[0]])
      output = output.at[-2 * halo_extents[0]:-halo_extents[0]].add(
          output[-halo_extents[0]:])
      ## Apply correction along y
      output = output.at[:, halo_extents[1]:2 * halo_extents[1]].add(
          output[:, :halo_extents[1]])
      output = output.at[:, -2 * halo_extents[1]:-halo_extents[1]].add(
          output[:, -halo_extents[1]:])
...
def infer_sharding_from_operands(halo_extents, halo_periods, reduce_halo, mesh,
