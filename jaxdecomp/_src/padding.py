...
class SlicePaddingPrimitive(CustomParPrimitive):
    ...
    @staticmethod
    def impl(arr : ArrayLike, padding_width : int|tuple[int] , pdims : tuple[int] , mode : str = 'constant' ) -> Array:
        ...
        for dim , local_padding in enumerate(padding_width):
            ...
            if isinstance(local_padding, int):
                ...
                raise ValueError("Padding width must be an integer or a tuple of integers")
            ...
            match dim:
                ...
                case 0:
                    ...
                    arr = jnp.concatenate([jnp.pad(s, ((first, last), (0, 0), (0, 0)), mode = mode) for s in slices], axis = 0)
                case 1:
                    slices = jnp.array_split(arr, pdims[0], axis = 1)
                    arr = jnp.concatenate([jnp.pad(s, ((0, 0), (first, last), (0, 0)), mode = mode) for s in slices], axis = 1)
                case 2:
    ...
    @staticmethod
    def per_shard_impl(arr : ArrayLike, padding_width : int|tuple[int] , mode : str = 'constant') -> Array:
        return jnp.pad(arr, padding_width, mode = mode)
                    
    @staticmethod
    def infer_sharding_from_operands(padding_width : int|tuple[int] ,  pdims : tuple[int], mode : str , mesh : Mesh, arg_infos : Tuple[ShapeDtypeStruct], result_infos : Tuple[ShapedArray]):
        input_sharding = arg_infos[0].sharding
        return NamedSharding(input_sharding.mesh, P(*input_sharding.spec))

    @staticmethod
    def partition(padding_width : int|tuple[int] ,  pdims : tuple[int], mode : str , mesh : Mesh, arg_infos : Tuple[ShapeDtypeStruct], result_infos : Tuple[ShapedArray]):
        ...
        impl = partial(SlicePaddingPrimitive.per_shard_impl, padding_width = padding_width, mode = mode)
        return mesh , impl , output_sharding, (input_sharding,)

register_primitive(SlicePaddingPrimitive)

@partial(jit , static_argnums=(1,2,3))
def slice_pad(x : ArrayLike, padding_width : int|tuple[int] ,  pdims : tuple[int] , mode : str = 'constant') -> Array:
...
class SliceUnPaddingPrimitive(CustomParPrimitive):
    ...
    @staticmethod
    def impl(arr : ArrayLike, padding_width : int|tuple[int] , pdims : tuple[int]) -> Array:
    ...
    @staticmethod
    def infer_sharding_from_operands(padding_width : int|tuple[int] , pdims : tuple[int], mesh : Mesh, arg_infos : Tuple[ShapeDtypeStruct], result_infos : Tuple[ShapedArray]):
        input_sharding = arg_infos[0].sharding
        return NamedSharding(input_sharding.mesh, P(*input_sharding.spec))
    
    @staticmethod
    def partition(padding_width : int|tuple[int], pdims : tuple[int], mesh : Mesh, arg_infos : Tuple[ShapeDtypeStruct], result_infos : Tuple[ShapedArray]):

        # Only one non static input and one output
        input_sharding  = NamedSharding(mesh , P(*arg_infos[0].sharding.spec)) 
        output_sharding = NamedSharding(mesh , P(*result_infos.sharding.spec))
        impl = partial(SliceUnPaddingPrimitive.per_shard_impl, padding_width = padding_width)
        return mesh , impl , output_sharding, (input_sharding,)
...
def slice_unpad(arr : ArrayLike, padding_width : int|tuple[int] , pdims : tuple[int]) -> Array:
