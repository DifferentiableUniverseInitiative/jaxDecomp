...
parameters = [((1,size),"X_halo") , ((1,size),"Y_halo") , ((1,size),"XY_halo") 
              , ((size,1),"X_halo") , ((size,1),"Y_halo") , ((size,1),"XY_halo") 
              , (pencil_1,"X_halo") , (pencil_1,"Y_halo") , (pencil_1,"XY_halo") 
              , (pencil_2,"X_halo") , (pencil_2,"Y_halo") , (pencil_2,"XY_halo")]

@pytest.mark.parametrize("pdims , halo", parameters) # Test with Slab and Pencil decompositions
def test_empty_halo(pdims,halo):
  ...
  gathered_stripped_exchanged_array = multihost_utils.process_allgather(stripped_exchanged_array,tiled=True)
  gathered_unpadded_array = multihost_utils.process_allgather(unpadded_array,tiled=True)


  assert_allclose(gathered_array, gathered_unpadded_array, rtol=1e-10, atol=1e-10)
  assert_allclose(gathered_array, gathered_stripped_exchanged_array, rtol=1e-10, atol=1e-10)

  print(f"pdims {pdims} with {halo} OK!")

@pytest.mark.parametrize("pdims", [(1 , 4)]) # For simplicity, only test Z axis decomposition
def test_full_halo(pdims):

  print("*"*80)
  print(f"Testing with pdims {pdims}")

  global_shape = (16 , 16 , 16)  # These sizes are prime numbers x size of the pmesh
  global_array, mesh = create_spmd_array(global_shape, pdims)
  halo_size = 2
  ...
  with mesh:
    ...
    exchanged_reduced_array = jaxdecomp.halo_exchange(
        ...
        halo_periods=(True, True, True),
        reduce_halo=True)
    unpadded_reduced_array = slice_unpad(exchanged_reduced_array, padding, pdims)
    unpadded_updated_array = slice_unpad(updated_array, padding, pdims)


  # Gather array from all processes
  # gathered_array = multihost_utils.process_allgather(global_array,tiled=True)
  exchanged_gathered_array = multihost_utils.process_allgather(exchanged_array,tiled=True)
  periodic_exchanged_gathered_array = multihost_utils.process_allgather(periodic_exchanged_array,tiled=True)
  updated_gathered_array = multihost_utils.process_allgather(updated_array,tiled=True)
  gathered_exchanged_reduced_array = multihost_utils.process_allgather(unpadded_reduced_array,tiled=True)
  gathered_unpadded_updated_array = multihost_utils.process_allgather(unpadded_updated_array,tiled=True)
  # Get the slices using array_split
  gathered_array_slices = jnp.array_split(exchanged_gathered_array, size, axis=0)
  gathered_updated_slices = jnp.array_split(updated_gathered_array, size, axis=0)
  gathered_periodic_exchanged_slices = jnp.array_split(periodic_exchanged_gathered_array, size, axis=0)
  gathered_reduced_exchange_slices = jnp.array_split(gathered_exchanged_reduced_array, size, axis=0)
  gathered_unpadded_updated_slices = jnp.array_split(gathered_unpadded_updated_array, size, axis=0)
  ...
  for slice_indx, (periodic_exchanged_slice, exchanged_slice, reduced_slice, original_slice, unpadded_slice) in enumerate(gathered_arrays):
    ...
    assert_array_equal(
        exchanged_slice[halo_size:-halo_size],
        original_slice[halo_size:-halo_size])
    assert_array_equal(
        periodic_exchanged_slice[halo_size:-halo_size],
        original_slice[halo_size:-halo_size])
