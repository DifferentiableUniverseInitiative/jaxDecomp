...
def test_jit():
    # Perform distributed FFT
    karray = jax.jit(lambda x: jaxdecomp.fft.pfft3d(x, pdims=pdims, global_shape=global_shape))(array)
    
    # Perform inverse FFT
    rec_array = jax.jit(lambda x: jaxdecomp.fft.pifft3d(x, pdims=pdims, global_shape=global_shape))(karray)
...
def test_vmap():
    x = jax.random.normal(shape=[128, global_shape[0]//pdims[1],
                                      global_shape[1]//pdims[0],
                                      global_shape[2]], 
                                      key=jax.random.PRNGKey(0))
    # Local value of the array
    ...
    # Perform distributed FFT
    karray = jax.vmap(lambda x: jaxdecomp.fft.pfft3d(x, pdims=pdims, global_shape=global_shape))(array)
    
    # Perform inverse FFT
    rec_array = jax.vmap(lambda x: jaxdecomp.fft.pifft3d(x, pdims=pdims, global_shape=global_shape))(karray)
