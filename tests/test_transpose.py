...
x = jax.random.normal(shape=[global_shape[0]//pdims[1],
...
def test_x_y():
    ...
    """
    array_transposed = jaxdecomp.transposeXtoY(array, pdims=pdims, global_shape=global_shape)
    global_array_transposed = global_array.transpose([2,0,1])
    
    assert_allclose(array_transposed, 
                    global_array_transposed[:,rank*global_shape[0]//pdims[1]:(rank+1)*global_shape[0]//pdims[1]],
...
def test_y_z():
    ...
    array_transposed = jaxdecomp.transposeYtoZ(array_transposed, pdims=pdims, global_shape=global_shape)
    global_array_transposed = global_array.transpose([1,2,0])
    
    assert_allclose(array_transposed, 
                    global_array_transposed[rank*global_shape[1]//pdims[1]:(rank+1)*global_shape[1]//pdims[1]],
                    rtol=1e-10, atol=1e-10)
...
def test_z_y():
    ...
    array_transposed = jaxdecomp.transposeXtoY(array, pdims=pdims, global_shape=global_shape)
    array_transposed = jaxdecomp.transposeYtoZ(array_transposed, pdims=pdims, global_shape=global_shape)
    array_transposed = jaxdecomp.transposeZtoY(array_transposed, pdims=pdims, global_shape=global_shape)
...
def test_y_x():
    ...
    """
    array_transposed = jaxdecomp.transposeXtoY(array, pdims=pdims, global_shape=global_shape)
    array_transposed = jaxdecomp.transposeYtoX(array_transposed, pdims=pdims, global_shape=global_shape)
