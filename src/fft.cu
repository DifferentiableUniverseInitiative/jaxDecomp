...
namespace jaxdecomp
{
                                                             ...
                                                             bool forward, bool adjoint)
    {
        ...
        if (config.pdims[0] == 1)
        {
            ...
            CHECK_CUFFT_EXIT(cufftMakePlanMany(
                desc.cufft_plan_c2c_x, 2, n.data(), nullptr, 1, pinfo_x_c.shape[0] * pinfo_x_c.shape[1], nullptr, 1,
                pinfo_x_c.shape[0] * pinfo_x_c.shape[1], get_cufft_type_c2c(real_t(0)), pinfo_x_c.shape[2], &work_sz_c2c_x));
        }
        else
        {
            CHECK_CUFFT_EXIT(cufftMakePlan1d(desc.cufft_plan_c2c_x, gx, get_cufft_type_c2c(real_t(0)),
        ...
        if (config.pdims[1] == 1)
        {
            ...
            if (config.transpose_axis_contiguous[1])
            {
                ...
                CHECK_CUFFT_EXIT(cufftMakePlanMany(
                    desc.cufft_plan_c2c_y, 2, n.data(), nullptr, 1, pinfo_y_c.shape[0] * pinfo_y_c.shape[1], nullptr, 1,
            ...
            else
            {
                ...
                desc.slab_yz = false;
                CHECK_CUFFT_EXIT(cufftMakePlanMany(desc.cufft_plan_c2c_y, 1, &gy /* unused */, &gy, pinfo_y_c.shape[0], 1, &gy,
        ...
        else
        {
            if (config.transpose_axis_contiguous[1])
            {
                CHECK_CUFFT_EXIT(cufftMakePlan1d(desc.cufft_plan_c2c_y, gy, get_cufft_type_c2c(real_t(0)),
        ...
        {
            CHECK_CUFFT_EXIT(cufftMakePlan1d(desc.cufft_plan_c2c_z, gz, get_cufft_type_c2c(real_t(0)),
                                             pinfo_z_c.shape[1] * pinfo_z_c.shape[2], &work_sz_c2c_z));
        }
        else
        {
            CHECK_CUFFT_EXIT(cufftMakePlanMany(desc.cufft_plan_c2c_z, 1, &gz /* unused */, &gz,
               ...
               void **buffers)
    {
        ...
        cudecompGridDesc_t grid_desc_c; // complex grid
        CHECK_CUDECOMP_EXIT(cudecompGridDescCreate(handle, &grid_desc_c, &desc.config, nullptr));
        ...
        if (desc.forward)
        {
            ...
            if (!desc.slab_xy)
            {
                ...
                {
                    CHECK_CUFFT_EXIT(cufftXtExec(desc.cufft_plan_c2c_y, output, output, desc.adjoint ? CUFFT_INVERSE : CUFFT_FORWARD));
                }
                else
                {
                    for (int i = 0; i < pinfo_y_c.shape[2]; ++i)
                    {
                        CHECK_CUFFT_EXIT(cufftXtExec(desc.cufft_plan_c2c_y, output + i * (pinfo_y_c.shape[0] * pinfo_y_c.shape[1]),
                                                     output + i * (pinfo_y_c.shape[0] * pinfo_y_c.shape[1]), desc.adjoint ? CUFFT_INVERSE : CUFFT_FORWARD));
                    }
                }
            }
            ...
            if (!desc.slab_yz)
            {
                 ...
                 * Perform FFT along z
                 */
                CHECK_CUFFT_EXIT(cufftXtExec(desc.cufft_plan_c2c_z, output, output, desc.adjoint ? CUFFT_INVERSE : CUFFT_FORWARD));
            }
        }
        else
        {
            ...
            if (!desc.slab_xy)
            {
                if (desc.config.transpose_axis_contiguous[1] || desc.slab_yz)
                {
                    CHECK_CUFFT_EXIT(cufftXtExec(desc.cufft_plan_c2c_y, output, output, desc.adjoint ? CUFFT_FORWARD : CUFFT_INVERSE));
                }
                else
                {
                    for (int i = 0; i < pinfo_y_c.shape[2]; ++i)
                    {
                        CHECK_CUFFT_EXIT(cufftXtExec(desc.cufft_plan_c2c_y, output + i * (pinfo_y_c.shape[0] * pinfo_y_c.shape[1]),
                                                     output + i * (pinfo_y_c.shape[0] * pinfo_y_c.shape[1]), desc.adjoint ? CUFFT_FORWARD : CUFFT_INVERSE));
                    }
                }
            }
             ...
             */
            CHECK_CUFFT_EXIT(cufftXtExec(desc.cufft_plan_c2c_x, output, output, desc.adjoint ? CUFFT_FORWARD : CUFFT_INVERSE));
        }
    ...
    template std::pair<int64_t, fftDescriptor_t> get_fft3d_descriptor<float>(cudecompHandle_t handle,
                                                                             cudecompGridDescConfig_t config,
                                                                             bool forward, bool adjoint);
    template std::pair<int64_t, fftDescriptor_t> get_fft3d_descriptor<double>(cudecompHandle_t handle,
                                                                              cudecompGridDescConfig_t config,
                                                                              bool forward, bool adjoint);
