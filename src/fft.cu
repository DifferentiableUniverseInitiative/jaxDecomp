...
namespace jaxdecomp{
    ...
    template<typename real_t> std::pair<int64_t, fftDescriptor_t> get_fft3d_descriptor(cudecompHandle_t handle,
    ...
    template<typename real_t> void fft3d(cudecompHandle_t handle,
        ...
        */
        if(desc.forward){
            CHECK_CUFFT_EXIT(cufftXtExec(desc.cufft_plan_c2c_x, input, input, CUFFT_FORWARD));
            ...
            */
            ...
            if (desc.config.transpose_axis_contiguous[1] || desc.slab_yz) {
                CHECK_CUFFT_EXIT(cufftXtExec(desc.cufft_plan_c2c_y, output, output, CUFFT_FORWARD));
            } else {
            ...
            * Perform FFT along z
            */
            CHECK_CUFFT_EXIT(cufftXtExec(desc.cufft_plan_c2c_z, output, output, CUFFT_FORWARD));
            }
            ...
            */
            CHECK_CUFFT_EXIT(cufftXtExec(desc.cufft_plan_c2c_z, input, output, CUFFT_INVERSE));
            ...
            */
            if (!desc.slab_xy) {
            if (desc.config.transpose_axis_contiguous[1] || desc.slab_yz) {
                CHECK_CUFFT_EXIT(cufftXtExec(desc.cufft_plan_c2c_y, output, output, CUFFT_INVERSE));
            } else {
                for (int i = 0; i < pinfo_y_c.shape[2]; ++i) {
                CHECK_CUFFT_EXIT(cufftXtExec(desc.cufft_plan_c2c_y, output + i * (pinfo_y_c.shape[0] * pinfo_y_c.shape[1]),
            ...
            */
            CHECK_CUFFT_EXIT(cufftXtExec(desc.cufft_plan_c2c_x, output, output, CUFFT_INVERSE));
        }
